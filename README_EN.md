...
- not part of this release slice: having `~compare` automatically create/resume `current_decision.json` through the generic entry, a standalone `workflow-learning` runtime helper, and the `~go exec` develop bridge
...
- when triggered, the host should prefer `current_handoff.json.artifacts.decision_checkpoint / decision_submission_state`; `.sopify-skills/state/current_decision.json` remains the state fallback
...
- when triggered, the host should prefer `current_handoff.json.artifacts.clarification_form / clarification_submission_state`; `.sopify-skills/state/current_clarification.json` remains the state fallback
...
- `go_plan_runtime.py` now treats a pending clarification checkpoint as a successful planning outcome instead of a failure
