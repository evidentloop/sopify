...
- **Status**: W1 完成 / W2 完成 / Phase 0 完成 — W3.1 Qoder adapter 待执行
- **Next**: W3.1 Build Qoder PROTOCOL_VERIFIED Adapter
- **Task**: W3.1 → W3.2 → W3.3 → W3.5 → W3.6 → Finalize
...
- [ ] Depends: Phase 0
- [ ] Input: existing Codex/Claude PROTOCOL_VERIFIED host patterns, Copilot workspace-scope adapter
...
- [ ] Output: Qoder 注册进 `installer/hosts/__init__.py`
- [ ] Output: Qoder prompt asset 消费 Host Protocol Entry Contract（含 request admission + 4 步续接 + 读取预算 + sopify_writer 写回边界）
