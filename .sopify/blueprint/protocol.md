...
## 6. Integration Contract（外部能力接入契约）— *informative；Verifier 为 normative 例外；EAR [SUPERSEDED by P8]*

> 本节整体 informative。其中 **Verifier**（§6.Verifier）已升格为 normative（P1.5-D）；~~ExecutionAuthorizationReceipt~~ 在 P8 中标记为 [SUPERSEDED]（pre-execution authorization model 退场）；其余子段仍为 draft。
...
| 出口 | 承载内容 |
|------|---------|
| `receipt` | 过程审计回执（`plan/<id>/receipts/*.json`；~~原 ExecutionAuthorizationReceipt [SUPERSEDED by P8]~~） |
