...
- **Status**: Wave 0–5 已完成。PR #68 和 PR #69 已依次进入 `main`；Sopify `2026-07-21` Release 已发布为 Latest，GitHub Pages 已以 `main / root` 上线，中英文入口已通过浏览器验证。
- **Next**: 通过 `sopify_writer.finalize_plan` 写入最终证据并归档，然后将归档变更作为独立小 PR 交付。
- **Task**: 22/22。
...
- [x] Wave 5 Git 交付、统一发布、Pages 和归档完成。

## Next

发布与 Pages 证据已验证；执行显式 `~go finalize`，归档 Git 变更通过独立小 PR 进入 `main`。
