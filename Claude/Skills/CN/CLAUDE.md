...
说明：普通主链路不需要记住 `~go exec`；当 plan 达到 `ready_for_execution` 后，宿主应继续按 `confirm_execute` + 自然语言确认推进。
...
- 用户补充后，宿主应在同一工作区重新调用默认 runtime 入口，让 runtime 负责继续跑 planning；若恢复后 `current_clarification.json` 被清理，视为正常收口。
